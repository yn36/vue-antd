  ...
  actions: {
    ...
    isAuthed({ commit }) {
      return new Promise((resolve, reject) => {
        ...
        if (token) {
          request('/system/isAuthed', 'post').then(result => {
            if (result.success) {
              // Vue.ss.set(ACCESS_TOKEN, result.token, 7 * 24 * 60 * 60 * 1000)
              storage.set(ACCESS_TOKEN, result.token, 7 * 24 * 60 * 60 * 1000)
              commit('SET_TOKEN', result.token)
              commit('SET_INFO', result.principal)
              commit('SET_NAME', { name: result.principal.realName, welcome: welcome() })
      ...
      return new Promise((resolve, reject) => {
        axios({
          ...
          if (response.success) {
            ...
            commit('SET_NAME', { name: response.body.realName, welcome: welcome() })
            resolve(response)
          } else {
            notification.error({
              message: '错误',
